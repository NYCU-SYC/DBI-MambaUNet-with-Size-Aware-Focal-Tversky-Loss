...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
        ...
        # do NOT crop zero regions; keep original full shape
        # do NOT crop; keep full image
        shape_before_cropping = data.shape[1:]
        properties['shape_before_cropping'] = shape_before_cropping

        bbox = [[0, int(s)] for s in shape_before_cropping]
        properties['bbox_used_for_cropping'] = bbox
        properties['shape_after_cropping_and_before_resampling'] = data.shape[1:]

        # if seg is None (test case), create a pseudo segmentation for normalization
        if seg is None:
            nonzero_mask = np.any(data != 0, axis=0)
            seg = np.where(nonzero_mask, 0, -1)[None].astype(np.int8)

        # force XY to 512x512, keep the first spatial axis unchanged
        old_shape = np.array(data.shape[1:], dtype=np.int64)

        if len(old_shape) == 3:
            # assume [D, H, W] after transpose
            new_shape = np.array([old_shape[0], 512, 512], dtype=np.int64)

            target_spacing = list(original_spacing)
            target_spacing[0] = original_spacing[0]
            target_spacing[1] = original_spacing[1] * old_shape[1] / 512
            target_spacing[2] = original_spacing[2] * old_shape[2] / 512

        elif len(old_shape) == 2:
            # 2D case: [H, W]
            new_shape = np.array([512, 512], dtype=np.int64)

            target_spacing = list(original_spacing)
            target_spacing[0] = original_spacing[0] * old_shape[0] / 512
            target_spacing[1] = original_spacing[1] * old_shape[1] / 512

        else:
            raise RuntimeError(f"Unexpected spatial dimension: {old_shape}")
